...
class TestPerformanceRegression(unittest.TestCase):
    ...
    def _measure_performance_with_statistics(
        self, func, *args, iterations: int = 15
    ) -> dict[str, float]:
        """
    ...
    def test_pattern_analysis_performance_regression(self):
        ...
        performance = self._measure_performance_with_statistics(
            analyzer.identify_evolution_opportunities, patterns, iterations=15
        )
    ...
    def test_proposal_generation_performance_regression(self):
        ...
        performance = self._measure_performance_with_statistics(
            generator.generate_proposals, opportunities, iterations=15
        )
    ...
    def test_validation_performance_regression(self):
        ...
        performance = self._measure_performance_with_statistics(
            validator.validate_proposals, proposals, iterations=15
        )
    ...
    def test_end_to_end_pipeline_performance_regression(self):
        ...
        performance = self._measure_performance_with_statistics(
            full_pipeline, patterns, iterations=10  # Fewer iterations for full pipeline test
        )
        ...
        print(f"  ✅ Average: {performance['average_ms']:.2f} ± {performance['std_dev_ms']:.2f}ms")
        print(
            f"  ✅ Results: {result['opportunities']} → {result['proposals']} → {result['validated']}"
        )
    ...
    def test_scalability_characteristics(self):
        ...
        for size in test_sizes:
            ...
            performance = self._measure_performance_with_statistics(
                analyzer.identify_evolution_opportunities, test_patterns, iterations=5
            )
            ...
            print(
                f"  Size {size:3d}: {performance['average_ms']:6.2f}ms → {len(performance['result'])} opportunities"
            )
        ...
        if len(performance_data) >= 2:
            ...
            print(
                f"  ✅ Scaling: {time_ratio:.2f}x time for {size_ratio:.2f}x patterns (reasonable)"
            )
    ...
    def test_memory_efficiency_regression(self):
        ...
        print("  ✅ No memory issues detected over 10 iterations")
