...
class MetricsRequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle_metrics_request(self, start_time: float):
        ...
        try:
            ...
            logger.debug(f"Metrics request served in {duration*1000:.2f}ms " +
    ...
    def _handle_health_request(self, start_time: float):
        ...
        try:
            ...
            health_data = {
                "status": "healthy",
                "timestamp": time.time(),
                "metrics_exporter": "registered" if (hasattr(self.server, 'metrics_exporter') and 
                                                   self.server.metrics_exporter) else "not_registered"
            }
            
            if hasattr(self.server, 'get_server_stats'):
                health_data["server_stats"] = self.server.get_server_stats()
            
            response_text = f"# Prometheus Metrics Endpoint Health\n"
    ...
    def _handle_error_response(self, status_code: int, message: str, start_time: float):
        ...
        try:
            ...
            duration = time.perf_counter() - start_time
            logger.warning(f"Error response {status_code} served in {duration*1000:.2f}ms: {message}")
...
class MetricsEndpoint:
    ...
    def __init__(
        ...
        logger.info(f"MetricsEndpoint initialized on {host}:{port} " +
    ...
    def start(self) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                self._server._update_request_stats = self._update_request_stats
                self._server.get_server_stats = self.get_server_stats
        ...
        with self._lock:
            ...
            derived_stats = {
                ...
                'average_bytes_per_request': (
                    stats['total_bytes_served'] / total_requests 
                    if total_requests > 0 else 0.0
                ),
