...
class InputBuffer:
    ...
    def __init__(
        self, 
        batch_sender: Callable[[List[str]], List[Dict[str, Any]]],
    ...
    def _should_buffer(self) -> bool:
        ...
        recent_inputs = [
            ts for ts in self.input_timestamps 
            if time.time() - ts <= 1.0
        ]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "buffering_ratio": (
                self.stats["batched_inputs"] / max(1, self.stats["total_inputs"])
            )
        }
...
class OptimizedInputManager:
    ...
    def __init__(self, pokemon_client: Any, buffer_config: Optional[BufferConfig] = None):
        """
        ...
        self.buffer = InputBuffer(
            batch_sender=self._batch_sender_adapter,
