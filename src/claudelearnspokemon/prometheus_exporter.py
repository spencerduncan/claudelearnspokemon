...
from claudelearnspokemon.circuit_breaker import CircuitBreaker  # type: ignore
from claudelearnspokemon.health_monitor import HealthMonitor  # type: ignore
from claudelearnspokemon.process_metrics_collector import (  # type: ignore
...
class PrometheusMetricsExporter:
    ...
    def __init__(self, registry: CollectorRegistry | None = None):
        ...
        self._aggregated_collector: AggregatedMetricsCollector | None = None
    ...
    def register_circuit_breaker(self, breaker: CircuitBreaker, name: str | None = None):
        """
    ...
    def _update_process_metrics(self):
        ...
        for process_id, collector in self._process_collectors.items():
            try:
                ...
                self.process_uptime.labels(process_id=process_id_str).set(metrics.total_uptime)
        ...
        for monitor_name, monitor in self._health_monitors.items():
            try:
                ...
                if stats["total_checks"] > 0 and stats["average_check_time"] > 0:
                    self.health_monitor_check_duration.labels(monitor_name=monitor_name).observe(
                        stats["average_check_time"]
                    )
                ...
                if hasattr(monitor, "emulator_pool") and hasattr(
                    monitor.emulator_pool, "clients_by_port"
                ):
                    try:
                        ...
                        for port_str, emulator_data in health_report.get("emulators", {}).items():
                            ...
                            self.health_monitor_emulator_status.labels(
                                monitor_name=monitor_name, port=port_str, container_id=container_id
                            ).set(is_healthy)
                    except Exception as e:
                        logger.debug(
                            f"Could not get detailed health report for {monitor_name}: {e}"
                        )
    ...
    def get_export_stats(self) -> dict[str, Any]:
        ...
        with self._lock:
            avg_export_time = (
                self._export_duration_sum / self._export_count if self._export_count > 0 else 0.0
            )
